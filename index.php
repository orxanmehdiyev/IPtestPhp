...
<div id="icerik" class="tablokapsyici">
	...
	$IPSor                    = $db->prepare("SELECT
		...
		FROM
		ip
		LEFT JOIN istifademeksedi ON ip.IstifadeMeksediID  = istifademeksedi.IstifadeMeksediID  order by $Sirala LIMIT $SayfalamagaBaslanacaqKayidSayisi, $SayfaBasinaGosterilecekKayidSayisi
		");
	$IPSor->execute();
	if ($ToplamSay > 0) {
		?>
		<table class="table">
				...
				<?php while ($IPCek = $IPSor->fetch(PDO::FETCH_ASSOC)) {
					...
					<tr id="<?php echo "daimiseneddouble_" . $IPCek['IP_ID']; ?>" ondblclick="Bax(this.id)">
						...
						<td class=" <?php if ($IPCek['IP_NVR_Status']==1) {
							...
							<td>
								<a href="edit.php?ip=<?php echo $IPCek['IP_IP'] ?>"><button class="cedvel_ici_button cedvel_ici_yesil_button" ><span class="icon">&#9998;</span></button></a>
			...
			<div>
				<div>
					<?php if ($SayfaBasinaGosterilecekKayidSayisi == 20) { ?>
						<img class="hamisigoster" alt="Göster" onclick="HamisiniGoster()" src="img/asagiox.jpg">
					<?php } else { ?>
						<img class="hamisigoster" alt="gizle" onclick="NormalGoster()" src="img/yuxariox.jpg">
					<?php } ?>
				</div>
			<?php } else {
			} ?>
			<div class="SayfalamaAlaniKapsayici">
				<div class="SayfalamaAlaniIciMetinAlaniKapsayicisi">
					Cəmi:
					<?php echo $BulunanSayafaSayisi; ?> səyfədə,
					<?php echo $ToplamSay; ?> ədəd sənəd var
				</div>
				<?php if ($BulunanSayafaSayisi > 1) {
					...
					<div class="SayfalamaAlaniIciNumaralandirmaKapsayicisi">
						...
						if ($GelenSayfalama > 1) {
							...
							?>
							<span class='Passif' onclick="Seyfeleme(1)">
								<<< /span>
								<span class='Passif' onclick="Seyfeleme(<?php echo $SayfalamaIciSayfaDeyeriBirGeriAl; ?>)">
									< </span>
								<?php }
								for ($SayfalamaIcinSayfaIndexDeyeri = $GelenSayfalama - $SayfalamaSolVeSagButtonSayisi; $SayfalamaIcinSayfaIndexDeyeri <= $GelenSayfalama + $SayfalamaSolVeSagButtonSayisi; $SayfalamaIcinSayfaIndexDeyeri++) {
									if ($SayfalamaIcinSayfaIndexDeyeri > 0 and $SayfalamaIcinSayfaIndexDeyeri <= $BulunanSayafaSayisi) {
										if ($SayfalamaIcinSayfaIndexDeyeri == $GelenSayfalama) { ?>
											<span class='Aktive'>
												<?php echo $SayfalamaIcinSayfaIndexDeyeri . " "; ?>
											</span>
										<?php } else { ?>
											<span class='Passif' onclick="Seyfeleme(<?php echo $SayfalamaIcinSayfaIndexDeyeri; ?>)">
												<?php echo $SayfalamaIcinSayfaIndexDeyeri . " "; ?>
											</span>
										<?php }
									}
