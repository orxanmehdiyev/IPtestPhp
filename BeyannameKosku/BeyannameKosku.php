	...
	$IPSor                    = $db->prepare("SELECT
		...
		");
	$IPSor->execute(array(
		'istifademeksedi_IstifadeMeksediID'=>4));
			...
			<tbody>
				<?php while ($IPCek = $IPSor->fetch(PDO::FETCH_ASSOC)) {
					...
					<tr id="<?php echo "daimiseneddouble_" . $IPCek['IP_ID']; ?>" ondblclick="Bax(this.id)">
						...
						<td><?php echo $IPCek['TelefonNo']; ?></td>
						<td><?php echo $IPCek['IP_Qeyd']; ?></td>	
						<td>
							<a href="beyannemekosku.php?ip=<?php echo $IPCek['IP_IP'] ?>"><button class="cedvel_ici_button cedvel_ici_yesil_button" ><span class="icon">&#9998;</span></button></a>
