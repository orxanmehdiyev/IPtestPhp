	...
	$IPSor                    = $db->prepare("SELECT
		...
		");
	$IPSor->execute(array(
		'istifademeksedi_IstifadeMeksediID'=>2));
			...
			<tbody>
				<?php while ($IPCek = $IPSor->fetch(PDO::FETCH_ASSOC)) {
					...
					<tr id="<?php echo "daimiseneddouble_" . $IPCek['IP_ID']; ?>" ondblclick="Bax(this.id)">
						...
						<td>
							<a href="pc.php?ip=<?php echo $IPCek['IP_IP'] ?>"><button class="cedvel_ici_button cedvel_ici_yesil_button" ><span class="icon">&#9998;</span></button></a>
